...
const MultipleSelect = (data,activeScreen,setActiveScreen,setScreen) => {
  ...
  const multiSelectHandler=(event,buttonIndex) => {
   ...
   if(useranswer===data.data[1].correct_answer[0]){
     data.setActivescreen_score(data.activescreen_score+1)
     setMcqscore(msqScore+1);
   }else if(useranswer===data.data[1].correct_answer[1]){
     data.setActivescreen_score(data.activescreen_score+1)
     setMcqscore(msqScore+1);
     }else if (useranswer===data.data[1].answers[2]){
       if(msqScore>=1)
       data.setActivescreen_score(data.activescreen_score-1)
     }else if (useranswer===data.data[1].answers[3]){
       if(msqScore>=1)
       data.setActivescreen_score(data.activescreen_score-1)
     }
   }
   const nextHandler = () => {
    ...
    return (
      <div>
        ...
        {(data.data[1].answers.map((answer,index)=> <button key={index} className = {buttonSelected[index]? "question-choices-tick": "question-choices"}  onClick={(e) => multiSelectHandler(e,index)}>{answer}</button>))}
